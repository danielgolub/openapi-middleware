...
export default class SecurityValidator {
  ...
  constructor(securitySchemes, endpointSecurityDefinition = [], securityCallbacks = {}) {
    ...
    this.validateClassParams();
    this.setupTestSchema();
  }

  /**
   * Validate input given to the class
   * @private
   */
  validateClassParams() {
    ...
    // all endpoint security definitions to be checking
    const endpointSecurityList = this.endpointSecurityDefinition.map((endpointSecurityObj) => {
    ...
    const securityCallbacksValidation = {
      ...
      exec(callbackDef, data) {
        if (endpointSecurityList.some((securityHandler) => !Object.keys(data).includes(securityHandler))) {
